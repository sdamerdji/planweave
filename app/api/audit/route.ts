...
async function searchNews(nonProfit: string, jurisdiction: string): Promise<any> {
  try {
...
async function evaluateNonProfit(nonProfit: string, newsResults: any): Promise<string> {
  try {
...
${newsContent}`;
    ...
    return response.choices[0].message.content?.trim() || "No clear evidence of issues found.";
  } catch (error) {
...
async function analyzeHudDocument(
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      sendProgress(`Found ${relevantParts.length} relevant project sections. Processing ${partsToProcess.length}.`);
        ...
        for (const nonProfit of nonProfits) {
          ...
          if (nonProfitCache[nonProfit]) {
            ...
            if (newsResults.results && newsResults.results.length > 0) {
              sendProgress(`Found ${newsResults.results.length} news articles.`);
